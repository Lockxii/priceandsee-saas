...
export function DashboardHeader({ plan }: { plan?: string | null }) {
  ...
  return (
    <header className="h-[4.25rem] shrink-0 flex items-center justify-between gap-4 px-6 sm:px-8 bg-white/95 backdrop-blur-sm border-b border-[var(--dash-border)] sticky top-0 z-20">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <span className="hidden sm:inline rounded-full border border-[var(--dash-border)] bg-white px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--dash-accent)]">
          {formatPlan(plan)}
        </span>
        <Link
          href="?settings=true"
          scroll={false}
          className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--dash-border)] text-[#8a7668] hover:bg-[#fffdfb] hover:text-[var(--dash-ink)] transition-colors"
